...
squares = [n*n for n in range(9)]
cubes = [n**3 for n in range(6)]
squares_of_evens = [n*n for n in range(9) if n % 2 == 0]

def palindromes(my_list):
    return [x + x[::-1] for x in my_list]

def starting_with(ltr, lst):
    return [name for name in lst if name[:1]==ltr.upper()]
